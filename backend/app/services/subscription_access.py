...
def get_active_subscription_for_entreprise(
    ...
    # Hna l-farq: n-khdmu b naive UTC ila kant l-DB naive
    now = datetime.utcnow() 
    
    candidates: list[Subscription] = []
    for sub in rows:
        # T-akked anaho end_date machi naive ila knti m-st3mel timezone.utc
        if sub.end_date:
            # Ila sub.end_date fih timezone w "now" la, ghadi i-tra crash
            # Bach t-hni rasek, redhom b-juj naive (ila konti f server wahed)
            expiry = sub.end_date.replace(tzinfo=None) if sub.end_date.tzinfo else sub.end_date
            if expiry < now:
                continue
    ...
    # Bach n-khtaru l-ahssen (Unlimited > Limited > Trial)
    return max(
        candidates,
        key=lambda s: (plan_rank(s.plan_code), s.updated_at.timestamp() if s.updated_at else 0),
    )
