...
def _stripe_resource_to_plain_dict(obj: Any) -> Dict[str, Any]:
    """
    Normalise les réponses Stripe (StripeObject) en dict Python.

    Depuis stripe-python v10+, les objets API ne sont plus des ``dict`` et n’ont
    pas ``.get()`` ; ``dict(obj)`` peut échouer. La méthode officielle est
    ``to_dict()`` (sérialisation récursive).
    """
    if obj is None:
        return {}
    ...
    if callable(to_dict):
        return to_dict()
    raise TypeError(f"Impossible de convertir en dict : {type(obj)!r}")
...
def sync_subscription_from_stripe_obj(
    ...
    *,
    ...
    sub_dict = _stripe_resource_to_plain_dict(sub_dict)
    sub_id = sub_dict.get("id")
    if not sub_id:
        logger.warning("sync_subscription: objet subscription sans id")
        return

    meta = dict(sub_dict.get("metadata") or {})
...
def refresh_subscription_row_from_api(db: Session, settings: Settings, subscription_id: str) -> None:
    ...
    sync_subscription_from_stripe_obj(db, settings, _stripe_resource_to_plain_dict(sub))


def handle_checkout_session_completed(db: Session, settings: Settings, session: Any) -> None:
    configure_stripe(settings)
    session = _stripe_resource_to_plain_dict(session)
    ...
    sub_dict = _stripe_resource_to_plain_dict(sub)
...
def confirm_checkout_session_for_entreprise(
    ...
    session_raw = stripe.checkout.Session.retrieve(
        sid,
        expand=["subscription", "line_items"],
    )
    session = _stripe_resource_to_plain_dict(session_raw)
...
def handle_invoice_payment_succeeded(db: Session, settings: Settings, invoice: Any) -> None:
    configure_stripe(settings)
    invoice = _stripe_resource_to_plain_dict(invoice)
...
def handle_invoice_payment_failed(db: Session, settings: Settings, invoice: Any) -> None:
    configure_stripe(settings)
    invoice = _stripe_resource_to_plain_dict(invoice)
    sub_id = invoice.get("subscription")
    if not sub_id:
